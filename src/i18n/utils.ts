...
export function stripBaseAndLocale(
  ...
  return new URL(path.slice(prefix.length), "https://example.com").pathname;
}
...
export function buildPrefix(
  ...
  return new URL(baseWithLocale, "https://example.com").pathname;
}

export function getSlugWithoutLocale(slug: string) {
  const resolveSlug = slug.startsWith("/") ? slug.slice(1) : slug;
  const slugParts = resolveSlug.split("/");
  if (
    slugParts.length > 1 &&
